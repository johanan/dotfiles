...
if ! pkg-config --exists "hyprwayland-scanner >= 0.4.4" 2>/dev/null; then
...
if ! pkg-config --exists "hyprutils >= 0.8.0" 2>/dev/null; then
    ...
    git clone --depth 1 --branch v0.8.1 https://github.com/hyprwm/hyprutils.git
    cd hyprutils
    cmake --no-warn-unused-cli -DCMAKE_BUILD_TYPE:STRING=Release -S . -B ./build
    cmake --build ./build --config Release -j`nproc 2>/dev/null || getconf _NPROCESSORS_CONF`
    sudo cmake --install ./build
...
# Build hyprlang
if ! pkg-config --exists "hyprlang >= 0.6.0" 2>/dev/null; then
    echo "Building hyprlang..."
    git clone --depth 1 --branch v0.6.3 https://github.com/hyprwm/hyprlang.git
    cd hyprlang
    cmake --no-warn-unused-cli -DCMAKE_BUILD_TYPE:STRING=Release -S . -B ./build
    cmake --build ./build --config Release -j`nproc 2>/dev/null || getconf _NPROCESSORS_CONF`
    sudo cmake --install ./build
...
# Build hyprgraphics
if ! pkg-config --exists "hyprgraphics" 2>/dev/null; then
    echo "Building hyprgraphics..."
    git clone --depth 1 --branch v0.1.5 https://github.com/hyprwm/hyprgraphics.git
    cd hyprgraphics
    cmake --no-warn-unused-cli -DCMAKE_BUILD_TYPE:STRING=Release -S . -B ./build
    cmake --build ./build --config Release -j`nproc 2>/dev/null || getconf _NPROCESSORS_CONF`
    sudo cmake --install ./build
...
if ! pkg-config --exists "hyprland-protocols >= 0.6.0" 2>/dev/null; then
    echo "Building hyprland-protocols..."
    git clone --depth 1 --branch v0.6.4 https://github.com/hyprwm/hyprland-protocols.git
...
# Build sdbus-c++
if ! pkg-config --exists "sdbus-c++ >= 2.0.0" 2>/dev/null; then
...
if ! command -v hyprlock &> /dev/null; then
    ...
    git clone --depth 1 --branch v0.9.0 https://github.com/hyprwm/hyprlock.git
    cd hyprlock
    cmake --no-warn-unused-cli -DCMAKE_BUILD_TYPE:STRING=Release -S . -B ./build
...
if ! command -v hypridle &> /dev/null; then
    ...
    git clone --depth 1 --branch v0.1.6 https://github.com/hyprwm/hypridle.git
    cd hypridle
